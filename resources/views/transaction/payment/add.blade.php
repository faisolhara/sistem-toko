...
<div class="row">
    <div class="col-sm-12">
        <div class="card-box">
        <h4 class="m-t-0 header-title"><b>{{ $title.' '.trans('menu.payment') }} <User></User></b></h4>
            <form class="form-horizontal" role="form" method="post" action="{{ route('payment-save') }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                <div class="row">
                    <div class="col-md-12">
                        <div class="p-20">
                            ...
                            <div class="col-md-6">
                                <input type="hidden" name="id" id="id" class="form-control" value="{{ count($errors) > 0 ? old('id') : $model->payment_header_id }}">
                                <div class="form-group {{ $errors->has('paymentNumber') ? 'has-error' : '' }}">
                                    <label class="col-md-4 control-label">{{ trans('fields.payment-number') }}</label>
                                    <div class="col-md-8">
                                        <input type="text" name="paymentNumber" class="form-control" value="{{ count($errors) > 0 ? old('paymentNumber') : $model->payment_number }}" readonly>
                                ...
                                <?php
                                    $paymentDate = new \DateTime($model->created_date);
                                ?>
                                <div class="form-group {{ $errors->has('paymentDate') ? 'has-error' : '' }}">
                                    <label class="col-md-4 control-label">{{ trans('fields.payment-date') }} *</label>
                                    <div class="col-md-8">
                                        <input type="text" name="paymentDate" class="form-control" value="{{ count($errors) > 0 ? old('paymentDate') : $paymentDate->format('d-m-Y') }}" readonly>
                                ...
                                <div class="form-group {{ $errors->has('descriptionHeader') ? 'has-error' : '' }}">
                                    ...
                                    <div class="col-md-8">
                                        <textarea name="descriptionHeader" id="descriptionHeader" class="form-control" rows="5">{{ count($errors) > 0 ? old('descriptionHeader') : $model->description }}</textarea>
                            ...
                            <div class="col-md-6">
                                <div class="form-group {{ $errors->has('totalPriceHeader') ? 'has-error' : '' }}">
                                    <label class="col-md-4 control-label">{{ trans('fields.total-price') }}</label>
                                    <div class="col-md-8">
                                        <input type="text" name="totalPriceHeader" id="totalPriceHeader" class="form-control text-right autonumber" value="{{ count($errors) > 0 ? old('totalPriceHeader') : $model->total_payment }}" readonly>
                                        @if($errors->has('totalPriceHeader'))
                                            <span class="help-block">{{ $errors->first('totalPriceHeader') }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="form-group {{ $errors->has('totalPayment') ? 'has-error' : '' }}">
                                    <label class="col-md-4 control-label">{{ trans('fields.total-payment') }}</label>
                                    <div class="col-md-8">
                                        <input type="text" id="totalPayment" name="totalPayment" class="form-control text-right autonumber" value="{{ count($errors) > 0 ? old('totalPayment') : $model->total_payment }}" >
                                        @if($errors->has('totalPayment'))
                                            <span class="help-block">{{ $errors->first('totalPayment') }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="form-group {{ $errors->has('returnPayment') ? 'has-error' : '' }}">
                                    <label class="col-md-4 control-label">{{ trans('fields.return-payment') }}</label>
                                    <div class="col-md-8">
                                        <input type="text" id="returnPayment" name="returnPayment" class="form-control text-right autonumber" value="{{ count($errors) > 0 ? old('returnPayment') : $model->return_payment }}" readonly>
                                        @if($errors->has('returnPayment'))
                                            <span class="help-block">{{ $errors->first('returnPayment') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            @if(empty($model->payment_header_id))
                            ...
                            <table class="table table-bordered table-hover table-striped m-0" id="table-detail" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    <tr data-index="{{ $indexDetail }}">
                                        ...
                                        <td class="text-right">{{ old('quantity')[$i] }}</td>
                                    ...
                                    <tr data-index="{{ $indexDetail }}">
                                        ...
                                        <td class="text-right">{{  number_format($line->quantity * $line->price) }}</td>
                                        <td>{{ $line->description }}</td>
                                        <input type="hidden" name="idDetail[]" value="{{ $line->payment_line_id }}">
                                        ...
                                        <input type="hidden" name="quantity[]" value="{{ number_format($line->quantity) }}">
                                        <input type="hidden" name="price[]" value="{{ number_format($line->price) }}">
                                        <input type="hidden" name="totalPrice[]" value="{{ number_format($line->quantity * $line->price) }}">
                    ...
                    <div class="col-sm-12 data-table-toolbar text-right">
                        <div class="form-group">
                            <a href="{{ url('payment') }}" class="btn btn-sm btn-warning"><i class="fa fa-reply"></i> {{ trans('fields.cancel') }}</a>
                            @if(empty($model->payment_header_id))
                                ...
                                <form  role="form" id="add-form" class="form-horizontal" method="post" action="">
                                    ...
                                    <div class="col-sm-12 portlets">
                                        ...
                                        <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                                            <label for="quantity" class="col-sm-4 control-label">{{ trans('fields.quantity') }} *</label>
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control currency text-right autonumber" data-a-sep="." data-a-dec="," id="quantity" name="quantity" value="">
                                                <span class="help-block"></span>
                                            </div>
                                        </div>
...
$(document).on('ready', function(){
    ...
    $('#add-detail').on('click', function() {
        ...
        if (quantity == '' || quantity <= 0) {
            ...
            $('#quantity').parent().find('span.help-block').html('Item is required');
            error = true;
        } else {
            $('#quantity').parent().parent().removeClass('has-error');
            $('#quantity').parent().find('span.help-block').html('');
        }
        ...
        ' <input type="hidden" nam e="itemId[]" value="' + itemId + '">' +
