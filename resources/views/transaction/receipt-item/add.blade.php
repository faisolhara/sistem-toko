...
<div class="row">
    <div class="col-sm-12">
        <div class="card-box">
        <h4 class="m-t-0 header-title"><b>{{ $title.' '.trans('menu.receipt-item') }} <User></User></b></h4>
            <form class="form-horizontal" role="form" method="post" action="{{ route('receipt-item-save') }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                <div class="row">
                    <div class="col-md-12">
                        <div class="p-20">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="header">
                                    <div class="col-md-6">
                                        <input type="hidden" name="id" id="id" class="form-control" value="{{ count($errors) > 0 ? old('id') : $model->receipt_item_header_id }}">
                                        <div class="form-group {{ $errors->has('receiptNumber') ? 'has-error' : '' }}">
                                            <label class="col-md-4 control-label">{{ trans('fields.receipt-number') }}</label>
                                            <div class="col-md-8">
                                                <input type="text" name="receiptNumber" class="form-control" value="{{ count($errors) > 0 ? old('receiptNumber') : $model->receipt_item_number }}" readonly>
                                        ...
                                        <div class="form-group {{ $errors->has('receiptDate') ? 'has-error' : '' }}">
                                            <label class="col-md-4 control-label">{{ trans('fields.receipt-date') }} *</label>
                                            <div class="col-md-8">
                                                <div class="input-group">
                                                    <input name="receiptDate" id="receiptDate" type="text" class="form-control datepicker-autoclose" style="position: relative; z-index: 100000;" value="{{ $receiptDate !== null ? $receiptDate->format('d-m-Y') : '' }}">
                                            ...
                                            <div class="col-md-8">
                                                <select class="form-control select2" name="supplierId" id="supplierId">
                                                    <option value="">{{ trans('fields.please-select') }}</option>
                                                        ...
                                                        <?php 
                                                        $supplierId = count($errors) > 0 ? old('supplierId') : $model->supplier_id;
                                                        ?>
                                                        <option value="{{ $option->supplier_id }}" {{ $supplierId == $option->supplier_id ? 'selected' : '' }}>{{ $option->supplier_name }}</option>
                                                        @endforeach
                                                </select>
                                                @if($errors->has('supplierId'))
                                                    <span class="help-block">{{ $errors->first('supplierId') }}</span>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                <div class="tab-pane" id="lines">
                                    @if(empty($model->receipt_item_header_id))
                                    ...
                                    <table class="table table-bordered table-hover table-striped m-0" id="table-detail" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            ...
                                            <tr data-index="{{ $indexDetail }}">
                                                ...
                                                <td class="text-right">{{ old('receiptQuantity')[$i] }}</td>
                                                ...
                                                <input type="hidden" name="receiptQuantity[]" value="{{ old('receiptQuantity')[$i] }}">
                                            ...
                                            <tr data-index="{{ $indexDetail }}">
                                                ...
                                                <td class="text-right">{{  number_format($line->receipt_quantity) }}</td>
                                                <td class="text-right">{{  number_format($line->price) }}</td>
                                                <td class="text-right">{{  number_format($line->receipt_quantity * $line->price) }}</td>
                                                <td>{{ $line->description }}</td>
                                                <input type="hidden" name="idDetail[]" value="{{ $line->receipt_item_line_id }}">
                                                ...
                                                <input type="hidden" name="receiptQuantity[]" value="{{ number_format($line->receipt_quantity) }}">
                                                <input type="hidden" name="price[]" value="{{ number_format($line->price) }}">
                                                <input type="hidden" name="totalPrice[]" value="{{ number_format($line->receipt_quantity * $line->price) }}">
                    ...
                    <div class="col-sm-12 data-table-toolbar text-right">
                        <div class="form-group">
                            <a href="{{ url('receipt-item') }}" class="btn btn-sm btn-warning"><i class="fa fa-reply"></i> {{ trans('fields.cancel') }}</a>
                            @if(empty($model->receipt_item_header_id))
                            ...
                            <div id="horizontal-form">
                                <form  role="form" id="add-form" class="form-horizontal" method="post" action="">
                                    ...
                                    <div class="col-sm-12 portlets">
                                        ...
                                        <div class="form-group {{ $errors->has('receiptQuantity') ? 'has-error' : '' }}">
                                            <label for="receiptQuantity" class="col-sm-4 control-label">{{ trans('fields.quantity') }} *</label>
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control currency text-right autonumber" data-a-sep="." data-a-dec="," id="receiptQuantity" name="receiptQuantity" value="">
                                                <span class="help-block"></span>
                                            </div>
                                        </div>
...
$(document).on('ready', function(){
    ...
    $('#receiptQuantity').on('keyup',function(){
        changeTotalPrice();
    });
    ...
    $('#add-detail').on('click', function() {
        ...
        if (receiptQuantity == '' || receiptQuantity <= 0) {
            $('#receiptQuantity').parent().parent().addClass('has-error');
            $('#receiptQuantity').parent().find('span.help-block').html('Item is required');
            error = true;
        } else {
            $('#receiptQuantity').parent().parent().removeClass('has-error');
            $('#receiptQuantity').parent().find('span.help-block').html('');
        }
        ...
        '<td class="text-right">' + receiptQuantity + '</td>' +
        ...
        ' <input type="hidden" name="receiptQuantity[]" value="' + receiptQuantity + '">' +
...
var changeTotalPrice = function(){
    ...
    receiptQuantity = $('#receiptQuantity').autoNumeric('get');

    totalPrice = price * receiptQuantity;
...
var editDetail = function() {
    ...
    var receiptQuantity       = $(this).parent().parent().find('[name="receiptQuantity[]"]').val();
    ...
    $('#receiptQuantity').val(receiptQuantity);
    $('#price').val(price);
    $('#totalPrice').val(totalPrice);
